...
async function testWebhookDirect() {
  ...
  try {
    ...
    console.log(`\n✅ Resposta HTTP ${res.status}:`, text);
    
    if (res.ok) {
      console.log('\n⏳ Webhook recebido! Aguardando 17 segundos para o debounce disparar...');
      await new Promise(r => setTimeout(r, 17000));
      console.log('✅ Tempo de debounce esgotado. Verifique se a mensagem chegou no WhatsApp e os logs do Next.js!');
    }
  } catch (e) {
    console.error('❌ Erro ao chamar o webhook:', e.message);
    console.log('\nVerifique se o Next.js está rodando com: npm run dev');
  }
}
