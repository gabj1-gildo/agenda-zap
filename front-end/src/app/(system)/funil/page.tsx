...
export default function FunilPage() {
  ...
  const { board, stats, isLoading: loading, mutate, moveLeadOptimistic } = useFunil(tenantId);
  ...
  const handleDrop = async (stageKey: StageKey, e: React.DragEvent) => {
    ...
    const toStage = STAGES.find(s => s.key === stageKey)!;

    moveLeadOptimistic(draggedId, fromStage.key, toStage.key, toStage.dbKey);
  };
