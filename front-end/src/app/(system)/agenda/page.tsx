...
function getMonthDays(base: Date) {
  ...
  // Calculate exactly how many rows we need (5 or 6)
...
export default function AgendaPage() {
  ...
  const [schedulingMode, setSchedulingMode] = useState("GERAL");
  ...
  useEffect(() => { 
    fetchAgenda(); 
    loadResources();
  }, [fetchAgenda, loadResources]);

  if (!tenantId) {
    return (
      <div className="max-w-7xl mx-auto flex flex-col items-center justify-center py-24 text-center">
        <div
          className="w-16 h-16 rounded-2xl flex items-center justify-center mb-4 bg-muted text-muted-foreground"
        >
          🗓
        </div>
      ...
      <div className="flex-1 min-h-0 overflow-hidden">
        {view === "month" && (
          <div className="bg-card border border-border rounded-2xl overflow-hidden h-full flex flex-col">
            ...
            {/* Day cells — flex-1 fills remaining space equally across rows */}
            <div
        ...
        {loading && (
          <div className="text-center text-sm text-muted-foreground py-4">Carregando agenda...</div>
        )}
      </div>
    </div>
  );
